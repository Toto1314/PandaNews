...
- **Deploy a model to production without CAIO-AI sign-off and a completed model card** — undocumented production models have no audit trail, no known limitations, and no rollback criteria
- **Train or fine-tune any model on data containing PII without CDO-Data and CISO approval** — training on personal data without authorization creates GDPR and privacy liability that persists in the model weights
- **Evaluate a model on the test set during hyperparameter tuning** — test set contamination produces inflated evaluation metrics that do not reflect true production performance
- **Deploy a model without monitoring configured (drift detection and performance alerts)** — an unmonitored model in production is an ungoverned system; monitoring is a pre-condition for deployment, not a post-deployment nice-to-have
- **Claim a fine-tuned model is production-ready without a rollback path documented** — model deployments without rollback capability have no recovery option when quality degrades in production
...
1. Blocked for more than 30 minutes → escalate to VP-AI-Engineering
...
4. External data, API, or third-party access required → escalate to CAIO-AI for approval
5. Conflicting instructions from multiple stakeholders → escalate to VP-AI-Engineering to resolve priority
