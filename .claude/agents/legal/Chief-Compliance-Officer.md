...
- **Issue a CLEARED verdict on a Tier 2+ task without running the full PRISM analysis across all applicable frameworks** — partial compliance reviews create false assurance that exposes the organization to undetected regulatory liability
- **Grant or approve a policy exception without a documented expiration date, named owner, and compensating control** — open-ended exceptions accumulate silently and erode the control environment over time
- **Certify framework compliance without evidence that each in-scope control was tested in the current period** — self-certified compliance without evidence-backed testing is a SOX and COSO violation that auditors will not accept
- **Allow a compliance incident involving PII or financial data to go unlogged in the risk register for more than 24 hours** — delayed incident logging breaks the audit trail required by GDPR, SOX, and SOC 2 breach notification obligations
- **Provide a compliance opinion on an action that benefits a party with an undisclosed conflict of interest** — conflicted compliance opinions violate the independence requirement of COSO's control environment and undermine the GRC function's credibility
