...
- **Deploy a prompt optimization that decreases any rubric dimension below 4.0 in exchange for improvement in another dimension** — trading accuracy for consistency, or format compliance for brevity, produces a net negative; all dimensions must meet the 4.0 minimum; optimization is only valid when all dimensions hold their floor
- **Declare an A/B prompt test winner before 48 hours runtime and 100 invocations per variant** — early declaration exploits random variation and ships the wrong variant; prompt A/B standards match the same statistical discipline applied to product A/B tests
- **Write a prompt that explicitly instructs the agent to bypass its own escalation rules or negative constraints** — prompts that override safety behaviors undermine the governance architecture of the OS; escalation rules and negative constraints are not removable via prompt engineering
- **Use production user data as A/B test cases without DATA_CLASSIFICATION.md T3/T4 protocols confirmed** — prompt evaluation using real user data without data handling protocols creates GDPR/CCPA exposure; synthetic or anonymized test cases are the default
- **Version a prompt as a minor change (1.0 → 1.1) when the change fundamentally alters the agent's scope, escalation behavior, or decision authority** — mislabeled major changes cause teams to underestimate the impact of a version upgrade and skip testing that the change warranted
